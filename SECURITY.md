...
- Use **`.env.example`** as a template: copy to `.env` and add your extraction API key.
...
2. Rotate the key in your provider’s dashboard and put the new key only in `.env` (never commit it).
